...
export function HumanLoopDashboard({ cases }: { cases: HumanCase[] }) {
  ...
  return <>
    ...
    <section className="case-metrics" aria-label="Tổng quan yêu cầu"><article><span className="case-metric-icon blue"><Icon name="file"/></span><p><small>Yêu cầu đang mở</small><strong>{items.filter((item) => item.status !== "Đã giải quyết").length}</strong><em>Trong hàng đợi hiện tại</em></p></article><article><span className="case-metric-icon violet"><Icon name="user"/></span><p><small>Chưa phân công</small><strong>{items.filter((item) => !item.owner).length}</strong><em>Cần người tiếp nhận</em></p></article><article><span className="case-metric-icon red"><Icon name="alert"/></span><p><small>P0 chưa xác nhận</small><strong>{items.filter((item) => item.priority === "P0" && !item.owner).length}</strong><em>Xử lý ngay</em></p></article><article><span className="case-metric-icon amber"><Icon name="clock"/></span><p><small>Sắp quá thời hạn</small><strong>2</strong><em>Trong 30 phút tới</em></p></article><article><span className="case-metric-icon green"><Icon name="check"/></span><p><small>Đã giải quyết</small><strong>{items.filter((item) => item.status === "Đã giải quyết").length}</strong><em>Trong phiên mô phỏng</em></p></article></section>
