...
export function ChatDrawer({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  return <><button className="drawer-scrim is-open" aria-label="Đóng trợ lý" onClick={onClose}/><aside className="chat-drawer is-open" role="dialog" aria-modal="true" aria-label="Trợ lý nhân viên">
    ...
    <div className="chat-body" aria-live="polite">{messages.length === 0 && <><div className="assistant-intro"><Icon name={mode === "patient" ? "users" : mode === "operations" ? "chart" : "book"}/><strong>{mode === "patient" ? selected ? `Đang tra cứu: ${selected.queue_number} · ${selected.patient_name}` : "Chưa có lượt khám thật" : "Trợ lý nghiệp vụ"}</strong><p>{mode === "patient" ? "Trạng thái được đọc trực tiếp từ hành trình người bệnh, không do AI suy đoán." : "Tra cứu thông tin hỗ trợ nhân viên."}</p></div><p className="suggestion-label">Gợi ý câu hỏi</p>{prompts[mode].map((prompt) => <button className="chat-suggestion" disabled={mode === "patient" && !selected} key={prompt} onClick={() => void ask(prompt)}>{prompt}<Icon name="arrow" size={17}/></button>)}</>}{messages.map((message, index) => <div key={index} className={`chat-bubble chat-bubble--${message.role}`}><ReactMarkdown remarkPlugins={[remarkGfm]}>{message.content}</ReactMarkdown>{message.role === "assistant" && message.content && <button type="button" className="speech-play" onClick={() => speech.speak(message.content)} aria-label={speech.speaking ? "Dừng đọc" : "Đọc câu trả lời"}><Icon name={speech.speaking ? "stop" : "volume"} size={15}/></button>}</div>)}{loading && <div className="typing" role="status">Đang cập nhật...</div>}</div>
    <form className="chat-input" onSubmit={(event) => { event.preventDefault(); void ask(); }}><label style={{ position: "relative" }}><span className="sr-only">Nhập câu hỏi</span><textarea style={{ paddingRight: 44 }} value={query} onChange={(event) => setQuery(event.target.value)} placeholder={speech.listening ? "Đang nghe..." : "Nhập câu hỏi của bạn..."} rows={2}/><button type="button" className={`speech-mic${speech.listening ? " is-listening" : ""}`} style={{ position: "absolute", right: 6, top: "50%", transform: "translateY(-50%)", width: 32, height: 32, border: 0, borderRadius: 9 }} onClick={speech.toggleListening} disabled={!speech.supported.stt || loading} aria-label={speech.listening ? "Dừng nghe" : "Nhập bằng giọng nói"} title={speech.listening ? "Dừng nghe" : "Nhập bằng giọng nói"}><Icon name={speech.listening ? "stop" : "microphone"} size={16}/></button></label><Button type="submit" disabled={!query.trim() || loading || (mode === "patient" && !selected)}><Icon name="arrow"/></Button>{speech.error && <small className="speech-error" role="alert">{speech.error}</small>}</form>
  </aside></>;
}
